...
enum TimelineTrackLayout {
    ...
    static func bands(in bounds: CGRect,
                      markerHeight: Double,
                      audioTracks: [String]) -> (marker: CGRect, video: CGRect, audio: [(track: String, rect: CGRect)]) {
        let marker = CGRect(x: 0, y: 0, width: bounds.width, height: markerHeight)
        let remaining = max(0, bounds.height - markerHeight)
        let video = CGRect(x: 0, y: markerHeight, width: bounds.width, height: remaining * 0.6)
        let audioTotal = remaining * 0.4
        guard !audioTracks.isEmpty else { return (marker, video, []) }
