...
enum SnittPalette {
    ...
    static let ink1 = NSColor(srgbRed: 0.102, green: 0.118, blue: 0.153, alpha: 1)

    /// Audio bands, the transport cluster, quiet buttons on ink, word chips.
    /// Replaces `grey(0.22)`.
    static let ink2 = NSColor(srgbRed: 0.137, green: 0.157, blue: 0.204, alpha: 1)

    /// Separators and borders drawn *on* ink. Replaces `grey(0.34)`.
    static let ink3 = NSColor(srgbRed: 0.227, green: 0.255, blue: 0.314, alpha: 1)
    ...
    static let signalBright = NSColor(srgbRed: 1.000, green: 0.722, blue: 0.302, alpha: 1)
    ...
    static let clockAmber = NSColor(srgbRed: 1.000, green: 0.851, blue: 0.627, alpha: 1)
    ...
    static let redBright = NSColor(srgbRed: 1.000, green: 0.420, blue: 0.420, alpha: 1)

    // MARK: - Slate and the playhead

    /// Lane labels, ruler timecodes, hairlines, and quiet text on ink.
    static let slateText = NSColor(srgbRed: 0.604, green: 0.639, blue: 0.710, alpha: 1)

    /// The playhead line and its caret — the brightest thing on the
    /// instrument, because it outranks everything it crosses. Replaces
    /// `grey(0.97)`.
    static let playheadInk = NSColor(srgbRed: 0.961, green: 0.965, blue: 0.973, alpha: 1)
    ...
    static let amberText = NSColor(name: "amberText") { appearance in
    ...
    static let redText = NSColor(name: "redText") { appearance in
        appearance.bestMatch(from: [.aqua, .darkAqua]) == .darkAqua
            ? redBright
    ...
    enum Swatch {
        static var ink0: Color { Color(nsColor: SnittPalette.ink0) }
        static var ink1: Color { Color(nsColor: SnittPalette.ink1) }
        static var ink2: Color { Color(nsColor: SnittPalette.ink2) }
        static var ink3: Color { Color(nsColor: SnittPalette.ink3) }
        static var signal: Color { Color(nsColor: SnittPalette.signal) }
        static var signalBright: Color { Color(nsColor: SnittPalette.signalBright) }
        static var clockAmber: Color { Color(nsColor: SnittPalette.clockAmber) }
        static var recordRed: Color { Color(nsColor: SnittPalette.recordRed) }
        static var redBright: Color { Color(nsColor: SnittPalette.redBright) }
        static var slateText: Color { Color(nsColor: SnittPalette.slateText) }
        static var playheadInk: Color { Color(nsColor: SnittPalette.playheadInk) }
        static var amberText: Color { Color(nsColor: SnittPalette.amberText) }
        static var redText: Color { Color(nsColor: SnittPalette.redText) }
    }
}
