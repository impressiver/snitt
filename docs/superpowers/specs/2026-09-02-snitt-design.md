...
- **Input a caller REPORTED is not captured input, and is drawn by default**
  (D105). Every rule above exists because Snitt caught something the person at
  the keyboard did not hand over: a token typed into a terminal is not a secure
  field, and the event tap saw it anyway. A reported click was handed over: the
  caller measured it, named it, and asked Snitt to put it on the recording's
  clock. Only reported clicks can be drawn at all, since a click the tap saw
  carries no position (`ClickOverlay`), so this default cannot surface anything
  its own caller did not supply.
...
- **Every failure names which of three things the caller should do** (D106).
  `invalid_arguments` means the call was wrong: change it and send it again.
  `unusable_recording` means the call was fine and this recording cannot serve
  it, so repeating it unchanged never will. `busy` means nothing is wrong and
  the same request will very likely work in a moment. What is left is
  `internal_error`, which now means "unexpected, no advice beyond the hint"
  rather than "unclassified". A hint that says "try again" under a code that
  says "something broke" is the contradiction this split removes, and both
  frontends carry it: the CLI in its JSON and its exit code, the MCP server in
  the text block and in `structuredContent` (D103).
...
- **D97 — one title row, and the side panel treated the way Xcode treats
  ...
  *Why it was not a small change.* The toolbar was a SwiftUI view inside an
  `NSHostingView`, and an `NSToolbar` is AppKit — so this was either SwiftUI's
  ...
  window is assembled rather than how it is painted.

  *The titlebar half was BUILT on 2026-09-20.* `EditorWindowToolbar` is a real
  `NSToolbar` whose items host the SwiftUI controls, the window is
  `.unifiedCompact` with a system-drawn title and subtitle, and
  `.fullSizeContentView`, `titlebarAppearsTransparent` and the hand-held
  `trafficLightInset` are gone. The absence marker is retired with it. **The
  side-panel half is still queued** — the rail remains a `VStack` with
  `ResizableDivider`, not an `NSSplitViewController` inspector item — and
  [#161](https://github.com/impressiver/snitt/issues/161) stays open for it.

  *What the hoist cost, recorded because it is the part that was underestimated.*
  `croppingActive`, `cropBox` and `showRail` were `@State` private to
  `EditorContentView`, and an `NSToolbarItem` cannot see a SwiftUI view's
  private storage — so the toolbar could not read whether crop mode was on, nor
  turn it off after applying. They moved to `EditorChromeState`, shared by the
  content view and the titlebar. That, not the items, was the change.
...
**Queued work lives in the issue tracker, not here (2026-09-17).** A decision
that has been made but not built gets a GitHub issue labelled `enhancement`,
and its row's status points at that issue. The row itself stays: other
decisions cross-reference it by number, and `SpecConformanceTests` fails if a
D-number does not resolve. This log records what was DECIDED and why; the
tracker records what is still OUTSTANDING. Keeping both in one table is what
let D55 and D62 sit marked "queued" for weeks after they shipped.
...
| D103 | **An MCP tool result carries `structuredContent`, not only prose** | §4.8 wrote an "agent-facing output contract: structured JSON on stdout, human-readable text on stderr, meaningful exit codes", and §15 scoped it to **the CLI's** JSON output, so the MCP server never had an output contract at all. That was invisible while MCP was the newer surface and became the problem once it was the primary one. `describe(_:)` encoded `.targets` and `.status` as JSON and rendered the rest as sentences, so an agent recovered a `sessionId` or `bundlePath` by pattern-matching prose — values that thirteen tool signatures take as INPUT. The sharpest case was `.inspected`, which dropped `health` and `git` entirely, from a struct whose own doc comment says it exists "so an agent can write something factually true in a pull request instead of narrating a recording it has never seen". The text block is unchanged, so hosts that ignore `structuredContent` keep working; the object mirrors what `snitt-cli` already writes for the same response, built from the same `Codable` value so the two cannot drift. **This is a gap being filled, not a violation being fixed** — an earlier draft of the audit claimed §4.8 was breached, and §15 refutes that | §4.8, §15, D51; `snitt-mcp/main.swift`, `InspectReport`; agent API audit 2026-09-18 | Decided | agent-surface-answering-in-prose |
| D104 | **A response carries what the caller cannot otherwise learn: dropped vocabulary, and grant state before the call** | Two agent-facing answers were unobtainable except by failing or by not finding out at all. **Truncation.** `Vocabulary`'s doc comment promises that "truncating is reported rather than silent" and `prepare` returns `(terms, dropped)` to keep it, but the only agent call site read `.terms` and threw the count away, so an agent that listed 150 terms got an ordinary success and never learned that 50 of them biased nothing: a promise kept by the function and broken by its caller, and the confidently-wrong outcome §8 forbids. `.started` now carries `vocabularyDropped`, `nil` when no vocabulary was sent and a count otherwise, so "sent none" and "sent some, kept all" stay different answers. **Consent.** `StatusInfo` carried recording/sessionID/elapsed/paused and nothing about grants, so an agent learned that agent recording was off, or that full-display capture was never enabled, by calling a tool and reading `consent_required`. That error is well-hinted, so this was a gap rather than a trap, but it costs a failed call on every cold start and arrives too late to plan around. `StatusInfo.consent` reports both switches plus D95's unattended grant, and `snitt_status` stays ungated so the pre-flight can never itself be refused. **The subtlety that shapes it**: `UnattendedRecordingGrant.status` returns `.off` both when a person never enabled unattended recording and when they did but agent recording is off, so reporting it directly would send an agent to fix the wrong switch and fail twice. `standingStatus` reports the grant on its own terms, `agentRecording` reports the switch, and `unattendedPermitted` is the composition, because `unattended != .off` is the wrong test and reads as the right one. Both fields are additive and Optional, proved rather than assumed by `ResponseWireCompatibilityTests`: `.stopped`'s "v2 has never shipped" reasoning expired when v2 shipped in 0.2.0 | §4.8, §5.3, §8, D95, D103; `Vocabulary.prepare`, `ConsentInfo`, `UnattendedRecordingGrant.standingStatus`; agent API audit 2026-09-18 | Decided | an-agent-learns-a-grant-by-failing |
| D105 | **Coordinates may be given in pixels, as long as the caller names the picture they are pixels of; and reported clicks are drawn by default** | Two findings with one shape: the agent surface asked an agent to restate what it already knew. `snitt_crop` took 0-1 fractions and RETURNED pixels, `snitt_report_input` took fractions and told the caller to "compute them from the element's position plus the browser's own chrome offset", and `CropRect.init` clamps rather than throws, so getting the arithmetic wrong succeeded quietly. Both now accept a `frameWidth`/`frameHeight` pair naming the window or the `snitt_screenshot` image the numbers were measured in, and refuse anything that leaves that frame instead of clamping it. **The stored `CropRect` stays fractional**, which was never the defect: a unit rect survives a change of source resolution, and an agent's own numbers come from spaces that differ from `capture.mov` anyway (a Retina capture is twice its window in points; the inline screenshot is downscaled to 1280px), so a fraction of a NAMED frame is the only form right in all of them. Neither frontend may read `capture.mov` (§4.9), which is why the caller names its own frame rather than being told Snitt's. Second half: `clicks` on `snitt_export` defaulted OFF, so an agent that followed the server's instructions and reported every input still shipped the unwatchable demo those instructions exist to prevent. §5.6's opt-in governs input Snitt CAUGHT; reported input was handed over by the caller, and only reported clicks can be drawn | §4.8, §4.9, §5.6, §8, D64, D72; `CoordinateFrame`, `ClickOverlay`, `CropRect`; agent API audit 2026-09-18, PR E and PR G | Decided | snitt-agent-api-audit |
| D106 | **`internal_error` splits three ways, and argument errors get a code at all** | `AutomationError.Code`'s own contract is "an agent branches on this, never on `message`", and two things defeated it. **Argument errors carried no code**: `MCPBridge.request` returned a bare `MCPBridgeError(String)` for a missing field, a wrong type, a string boolean, a non-positive scale, an inverted trim range or a partial crop rect, and `snitt-mcp` rendered that as plain text with `isError: true` while a real `AutomationError` rendered code-prefixed, so the largest class of real agent mistakes was the one class the taxonomy never reached. **And `internal_error` was a catch-all** across three unrelated answers (27 sites in `AutomationHost` alone) spanning caller-fixable validation (an unknown `reportInput` kind, an unsupported export format), permanently unusable data (a corrupt `edit.json`, `CompositionError.everythingCut`, `.noVideoTrack`, an input log with nothing to trim against) and transient conditions whose own hint said to try again in a moment (`stop`'s `.busy`, a screenshot before the first frame). One code and one exit code for "fix your request", "do not retry" and "wait and retry". The three new codes are named for the ANSWER, not the subsystem: `invalid_arguments` (17), `unusable_recording` (18), `busy` (19); `internal_error` keeps 16 and becomes the residue rather than the default. **Additive, and the exit table is frozen**: §15 makes those numbers a public interface, so D106 appends and moves nothing; what changes is which code a given failure carries. `MCPBridgeError` gains a defaulted `code` so forty-odd construction sites are untouched, and a bridge refusal now travels through the same `describe`/`structuredContent` path an app refusal does (D103), so the server answers in one error shape instead of two. **Unknown codes decode as `internal_error`** rather than throwing, which protects the NEXT addition and not this one: a client released before D106 still has the strict decoder and still throws `dataCorrupted` on the three new codes. Tolerable for D63's reason, that `snitt` and `snitt-mcp` ship inside `Snitt.app` and update with it, so only a loose binary on `PATH` is old enough to hit it, and it fails loudly as `malformedResponse` rather than silently | D103, §4.8, §11, §15; `AutomationError.Code`, `MCPBridgeError`, `AutomationHost`; agent API audit 2026-09-18 | Decided | one-code-for-three-different-instructions |
| D107 | **The agent API can caption a recording, and write the narration it captions** | The agent API audit's fourth finding, and the one with the widest gap between what the product does and what the agent surface reaches. `showSubtitles` and `showMarkers` drive burned-in captions and marker banners at export and appeared NOWHERE in `SnittAutomation`, so an agent could record speech, have it transcribed on device, and have no way to put a word of it on the picture, which is why the README demo plan resorted to hand-writing `edit.json`. Three moves. **Export gains `captions`/`markerBanners` as OPTIONAL overrides**, not flags: absent leaves the document's own setting standing, because a `false` default would make an agent's export silently drop captions a person had turned on in the editor (§8). They are applied through `EditDecisionList.drawing(captions:markerBanners:)` and never written back, an export is not an edit, and the editor's sheet writes them only because a person chose them in the document's own window. Named `captions`, not `subtitles`, because `subtitles` already means a `.vtt` sidecar built from MARKER transcripts while this burns the SPEECH transcript into the frames. **`snitt_transcript` reads what a recording says**, as pause-split LINES rather than a thousand word rows, each marked heard-or-written and audible-or-muted; a muted line is reported rather than filtered out, because every other surface drops it through `AudibleTranscript` and an agent would otherwise read a transcript and get an export missing the lines it just read. **`snitt_narrate` writes a line at a moment in the recording**, in scope against a review objection: §4.8's record-only bounds what Snitt does to the WORLD ("it does not click, type, or navigate, and it does not upload"), not what may enter a recording, and an agent has no voice, so authored narration is its microphone (D49 already grants agents markers carrying a transcript). It goes through D100's own `AuthoredNarration.words`/`.inserting`, so `isAuthored` is set by the one function that knows to set it and a written line can never be deleted by cutting footage it has none of. Snitt does not SPEAK it (D101 is queued), so both responses report `captionsEnabled` and say plainly when a write will reach nobody. Both verbs are gated on `allowsAgentAccess` like `.inspect`: a transcript is the most disclosive thing in a bundle. The protocol goes 3 -> 4, because these are new request cases and v3 shipped in v0.3.0 | §4.8, §8, D49, D100, D103; `EditDecisionList`, `AuthoredNarration`, `TranscriptReport`, `NarrationSummary`; agent API audit 2026-09-18 | Decided | an-agent-has-no-voice-so-a-written-line-is-its-microphone |
| D108 | **An agent can find the recordings it left behind** | The agent API audit enumerated all sixteen tools and found nothing that lists or removes a past recording: `snitt_status` reports only whether a recording is running NOW. §5.3's watchdog force-stops an abandoned agent's session and still finalises the bundle, recording `AuditOutcome.capped`, so an agent that crashes twice has two full-resolution videos it cannot find. **Verified while building this, and the plan marked it UNVERIFIED: the app's own UI does not surface them either.** Nothing under `Sources/` enumerates the output directory, `RecentDocuments.note` is called only after an editor window opens, and `RecordingCoordinator.openEditorIfHuman` deliberately never opens one for an agent's recording. So a capped bundle is absent from Open Recent, from the Dock reopen path and from every list the app draws; the only route to it is File > Open into a user-configurable directory. The gap is product-wide, and `snitt_list_recordings` / `snitt recordings list` closes the agent half: path, size, age, initiator and outcome, newest first, with `total` reported separately so a truncated list reads as truncated. **Capped-versus-completed needed somewhere to live.** `AuditRecord` is keyed by session id and carries no bundle path, so the audit trail could never be joined to a bundle; a capped bundle is finalised by exactly the same code as a clean stop, so nothing on disk distinguished them. `RecordingMetadata` gains an optional `outcome`, stamped by `AutomationHost` on both agent paths. Absent then means "did not end through the agent API", which is what a hotkey recording is. **Listing only, no delete**: removing somebody's recordings over a socket is a much larger decision, and the finding is that an agent cannot FIND its debris. `RecordingInventory` lives in `SnittAutomation` so CI runs its tests; a damaged `meta.json` is skipped rather than failing the listing, which is the opposite of `readEDL`'s discipline and right here, because one bad bundle must not make the other thirty unfindable | §4.8, §5.3, D107; `AutomationHost.expire`, `RecordingMetadata`, `RecordingInventory`, `OutputDirectorySettings`; agent API audit 2026-09-18 | Decided | the-watchdog-leaves-a-video-nobody-can-find |
| D110 | **A lane only for an audio source that was actually captured** | The editor drew a microphone lane on every recording, including the ones made with the microphone off, so an empty lane and a lane of silence were the same picture. **Track presence could never have answered this**: `EditDecisionList.fullRange()` writes a `TrackState` for both audio sources and `Recorder` writes it at `start()`, before one sample exists — so the filter that claimed to derive lanes from the recording had nothing to remove, and the test guarding it asserted that the filter drops a microphone from a list with no microphone in it, which no real recording ever produced. **`CaptureHealth`'s `nil` is the evidence.** `CaptureSession.start()` adds the `.microphone` and `.audio` stream outputs only when each option is on, so a source that was never enabled delivers no buffers and its RMS is `nil`, while one that WAS enabled has an RMS that may be zero. Those are different facts and the rule turns on exactly that. **Hidden only when never captured, never when merely quiet**: enabling a microphone is a choice a person made, and room tone or silence from a dead device is the answer to what it got — the second case being the one the lane most needs to report. A loudness threshold would collapse the lane of a microphone that worked, and `== 0` would fire on some silent recordings and not others, since digital silence through a float conversion is not reliably bit-exact. **Over-dubbing restores it**: a take lands on the microphone track (D102) and `meta.json` is written once at capture, so no health will ever mention audio the editor added afterwards; a take still OPEN counts too, or the lane would appear only once the recording into it had finished. A bundle with no health at all keeps every lane, because absent evidence is not evidence of absence. Presentational only — the states, the mix and `capture.mov`'s two audio tracks are untouched. Lives in `AudioTrackOrder` beside `CaptureHealth` so CI runs its tests, and is derived ONCE as `EditorTimelineState.audioLaneNames`, since the bands, the gutter rows and the height budget each used to derive it separately | §4.14, D56, D93, D102; `AudioTrackOrder.recorded`, `CaptureHealth.captured`, `EditorTimelineState.audioLaneNames`, `TimelineTrackLayout`; product-owner direction 2026-09-20 | Decided | an-empty-lane-and-a-lane-of-silence-were-the-same-picture |
