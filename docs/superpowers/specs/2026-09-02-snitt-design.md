...
4. **`auto-deep-trim` (D57) — BUILT, first slice (2026-09-08).** All five
   ...
   transcript for the timeline, so the detector reads those. Remaining: the
   CLI's `--preset` and per-criterion flags, and a higher-rate frame pass if
   the filmstrip's resolution proves too coarse in use.
