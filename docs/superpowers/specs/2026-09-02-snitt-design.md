...
4. **`auto-deep-trim` (D57) — BUILT, first slice (2026-09-08).** All five
   ...
   like every other document verb. Remaining: MCP exposure, so an agent can ask
   for it, and a higher-rate frame pass in the EDITOR if the filmstrip's
