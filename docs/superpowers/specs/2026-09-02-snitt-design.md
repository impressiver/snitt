...
3. **Transcription (D62) — BUILT, first slice (2026-09-08).** Transcribes on
   ...
   D57's use of the word spans as its dead-air signal, and **D73's pre-flight
   speaker-bleed warning** — when mic and system audio are both being captured
   and the default output device is built-in, the microphone will record the
   playback too, which the maintainer's own recording proved costs a whole
   take. Deterministic from CoreAudio's transport type, so it is a check rather
   than a heuristic.
