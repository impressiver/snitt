...
- **D93 — record a voiceover after the fact, in the editor.** Narrate over a
  ...
  *Why it is not just "another mic track":* §4.5's pristine-capture invariant
  says `capture.mov` is immutable and every edit is sidecar data, so a
  voiceover cannot be mixed into it. It is a NEW audio asset in the bundle
  alongside the existing tracks, and `AudioTrackOrder.canonical`
  (`[systemAudio, microphone]`) is currently a fixed two-element order that
  `MicrophoneTrackExtractor` indexes into by name — a third track has to join
  that vocabulary rather than be appended, or transcription silently reads the
  wrong one.

  *Edges, in the order they bite:* (1) it is recorded against OUTPUT time
  while every other track is in SOURCE time, so a later cut has to decide
  whether the voiceover moves with the picture or stays where it was spoken —
  the first genuinely ambiguous case in `Timebase`; (2) `EditDecisionList`
  needs a track state for it, which lands on `PassthroughEligibility` (an
  added track means an audio mix, so exports stop copying samples); (3)
  transcription would need to choose between the two spoken tracks, or merge
  them. *Unblocks a real thing:* it is the only way to narrate a recording an
  AGENT made, which today can never have a voiceover at all.
  `absent: VoiceoverTrack` (D91).
...
| D89 | **QUEUED, not built: the transcript becomes a timeline LANE under the audio bands, with words positioned at the times they are spoken** — replacing the side pane as the primary surface | Product-owner direction, filed as a future enhancement. The data is already there and already in the right shape: `TranscriptWord` carries `start`, D62's editing works word by word, and `TranscriptPlayhead.currentWordID` already resolves output seconds to a word — so this is a drawing problem, not a model one. **What makes it worth doing**: a transcript in a side pane is a list you read, while a transcript on the timeline is a MAP of the recording, which is what makes "cut from here to there" a text gesture instead of a drag — and D62 already argues text is the precise way to edit a recording. It also finishes what D79 started: a fold label describes a removed span in words, and a transcript lane describes every KEPT span in words. **The work is density.** A minute of speech is ~150 words and a timeline is ~1000px wide; the lane needs a level-of-detail rule (words when they fit, then phrases, then a speech/silence band) that stays legible at every zoom, and words must be positioned against OUTPUT time so a cut re-flows them rather than leaving them where the capture put them. `TimelineTrackLayout` gains a fourth band, and the pane's editing affordances have to survive the move or be kept alongside it | §4.5, §4.9, D50, D62, D79, D82; `TimelineView.swift`, `TimelineTrackLayout.swift`, `TranscriptPane.swift` | Queued (not built) | a-list-you-read-versus-a-map-of-the-recording |
