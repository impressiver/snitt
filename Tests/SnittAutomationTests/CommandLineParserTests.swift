...
func parsesExport() {
    guard case .success(.export(let path, let format, let out, let scale, let chapters, _, _, _)) =
...
func exportDefaults() {
    guard case .success(.export(_, _, _, let scale, let chapters, _, let maxSizeBytes, _)) =
        CommandLineParser.parse(["export", "/tmp/x.snitt", "--format", "mp4",
                                 "--out", "/tmp/demo.mp4"])
    else { Issue.record("parse failed"); return }
    #expect(scale == 1.0)
...
func exportAcceptsGif() {
    ...
    guard case .success(.export(_, let format, _, _, _, _, _, _)) = result else {
...
func maxSizeParsed() {
    let result = CommandLineParser.parse(
        ["export", "/tmp/b.snitt", "--format", "mp4", "--out", "/tmp/o.mp4",
         "--max-size", "10MB"])
    guard case .success(.export(_, _, _, _, _, _, let maxSize, _)) = result else {
        Issue.record("expected success, got \(result)"); return
    }
